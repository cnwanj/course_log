        ...
        <div class="layui-form-item">
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="depId" lay-verify="required" autocomplete="off" class="layui-input">
                </div>
            </div>
            <div class="layui-inline">
                <label class="layui-form-label">学院名称:</label>
                <div class="layui-input-inline">
                    <input type="text" name="depName" lay-verify="required" autocomplete="off" class="layui-input">
                </div>
            </div>
        </div>
        <div class="layui-form-item">
    ...
    <div class="layui-inline">
        <form class="layui-form" id="form1" lay-filter="dataFrm">
            <div class="layui-input-inline" style="width:200px;">
                <input type="text" name="depName" id="iname" list="nameList" lay-verify="required" autocomplete="off"
...
<div id="divTable">
    <div id="tabList">
        <table class="layui-table" lay-even lay-size="lg" lay-filter="demo">
            ...
            <tbody>
            <s:iterator value="pages.list" var="dep">
                <tr>
                    <td>${dep.depId}</td>
                    <td>${dep.depName}</td>
                    <th>
                        <button type="button" class="layui-btn layui-icon" name="${dep.depId}" onclick="edit(this)">
                            &#xe642; 编辑
                        </button>
                        <button type="button" class="layui-btn layui-btn-danger layui-icon" lay-submit
                                lay-filter="delete" name="${dep.depId}" onclick="del(this)">&#xe640; 删除
                        </button>
                    </th>
                </tr>
            </s:iterator>
    ...
    <div style="padding-left: 400px;">
        <a href="findAll_Department.action" class="layui-btn layui-btn-sm layui-btn-primary">首页</a>
        <a href="findAll_Department.action?pageCount=${pages.pageCount-1}"
           class="layui-btn layui-btn-sm layui-btn-primary">上一页</a>
        <a class="layui-btn layui-btn-sm">${pages.pageCount}</a>
        <a href="findAll_Department.action?pageCount=${(pages.pageCount+1)>=pages.totalPage?pages.totalPage:(pages.pageCount+1)}"
           class="layui-btn layui-btn-sm layui-btn-primary">下一页</a>
        <a href="findAll_Department.action?pageCount=${pages.totalPage}"
...
<script>
    ...
    layui.use(['element', 'jquery', 'layer', 'form', 'table'], function () {
        ...
        form.on('submit(save)', function (data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    url = "save_Department.action";   //提交表单跳转的路径
                }
            })
        });
        ...
        $("#iname").keyup(function () {
            ...
            var param = {depName: ($("#iname").val() == "" ? 0 : $("#iname").val())};
            $.post("searchDepartment.action", param, function (data) {
                var list = "";
                for (var i in data) {
                    list += "<option value='" + data[i].depName + "'></option>'"
                }
                $("#nameList").html(list);
            });
        });
        ...
        form.on('submit(search)', function () {
            ...
            $.post("searchDepartment.action", n, function (data) {
                if (data[0] != null) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        html += "<tr>" +
                            ...
                            "<th>" +
                            "<button type='button' class='layui-btn layui-icon' name='" + data[i].depId + "' onclick='edit(this)'>" + "&#xe642; 编辑" + "</button>" +
                            "<button type='button' class='layui-btn layui-btn-danger layui-icon' lay-submit lay-filter='delete' name='" + data[i].depName + "' onclick='del(this)'>" + "&#xe640; 删除" + "</button>" +
                            "</th>" +
                            "</tr>";
                    }
                    html += "</tbody>\n" +
    ...
    function edit(event) {
        window.location.href = "getOne_Department.action?depId=" + event.name;
    }

    function del(event) {
        var target = window.confirm("你真的要删除吗？");
        if (target) {
            window.location.href = "delete_Department.action?depId=" + event.name + "&pageCount=" +${pages.totalCount==6?(pages.pageCount-1):pages.pageCount};
        }
    }
</script>
</body>
